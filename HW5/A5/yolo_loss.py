...
class YoloLoss(nn.Module):
    ...
    def xywh2xyxy(self, boxes):
        ...
        ### CODE ###
        # Your code here
        boxes[:, 0] = boxes[:, 0]/self.S - boxes[:, 2]/2  
        boxes[:, 1] = boxes[:, 1]/self.S - boxes[:, 3]/2
        boxes[:, 2] = boxes[:, 0]/self.S + boxes[:, 2]/2 
        boxes[:, 3] = boxes[:, 1]/self.S + boxes[:, 3]/2
        return boxes
    ...
    def find_best_iou_boxes(self, box_pred_list, box_target):
        ...
        3) use xywh2xyxy to convert box format if necessary,
        ...
        for b in range(len(box_pred_list)):
            iou = compute_iou(self.xywh2xyxy(box_pred_list[b][:, :4]), box_target) # (N, N)     
        ...
        for n in range(N):
            ...
            best_boxes[n] = box_pred_list[torch.argmax(ious[n])][n]
    ...
    def forward(self, pred_tensor, target_boxes, target_cls, has_object_map):
        ...
        reg_loss = inv_N * self.get_regression_loss(best_boxes[:,:4], target_boxes)
        # compute contain_object_loss
        containing_obj_loss = inv_N * self.get_contain_conf_loss(best_boxes[:,4:5], best_ious)
        # compute final loss
